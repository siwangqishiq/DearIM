...
class TransMessageBuilder {
  static const int MAX_LENGHT = 5 * 1024;

  //创建透传消息
  static TransMessage? create(int toUid, String content , String? attach ,{bool offline = false}) {
    if (toUid <= 0) {
      LogUtil.errorLog("error uid for $toUid");
      return null;
    }

    if (content.length >= MAX_LENGHT || (attach != null && attach.length >= MAX_LENGHT)) {
...
class SendTransMessageHandler extends MessageHandler<SendTransMessageRespMsg> {
  ...
  void handle(IMClient client, SendTransMessageRespMsg msg) {
    ...
    if(msg.result != null){
      ...
      client.sendTransMessageCallbackMap[result.msgId]?.call(retTransMessage , result);
